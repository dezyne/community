...
struct ILedsGlue
{
  ...
  struct
  {
    std::function< void()> led1On;
    std::function< void()> led1Off;
    std::function< void()> led2On;
    std::function< void()> led2Off;
    std::function< void()> led3On;
    std::function< void()> led3Off;
    std::function< void()> led4On;
    std::function< void()> led4Off;
  } in;
  ...
  dzn::port::meta meta;
  inline ILedsGlue(const dzn::port::meta& m) : meta(m) {}

  void check_bindings() const
  {
    if (! in.led1On) throw dzn::binding_error(meta, "in.led1On");
    if (! in.led1Off) throw dzn::binding_error(meta, "in.led1Off");
    if (! in.led2On) throw dzn::binding_error(meta, "in.led2On");
    if (! in.led2Off) throw dzn::binding_error(meta, "in.led2Off");
    if (! in.led3On) throw dzn::binding_error(meta, "in.led3On");
    if (! in.led3Off) throw dzn::binding_error(meta, "in.led3Off");
    if (! in.led4On) throw dzn::binding_error(meta, "in.led4On");
    if (! in.led4Off) throw dzn::binding_error(meta, "in.led4Off");


  }
};

inline void connect (ILedsGlue& provided, ILedsGlue& required)
{
  provided.out = required.out;
  required.in = provided.in;
  provided.meta.requires = required.meta.requires;
